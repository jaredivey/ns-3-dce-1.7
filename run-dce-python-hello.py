...
file3 = "simulationResult/wns3-hello-resource-final.txt"
#clear the file
out3 = open(file3,'wb')
out3.close();
out3 = open(file3,'ab')

file4 = "simulationResult/wns3-hello-result-final.txt"
out4 = open(file4,'wb')
out4.close();
out4 = open(file4,'ab')
...
for langChoice in range(3,4):
    for loopCount in range(50,251,50):
        for iteration in range(0,1):
            out3.write("processing language = {0}, loopCount = {1}, iteration = {2}, at {3}\n".format(langChoice,loopCount,iteration,time.strftime('%Y-%m-%d %X',time.localtime(time.time()))))
            out3.flush()

            print "processing language = {0}, loopCount = {1}, iteration = {2}, at {3}".format(langChoice,loopCount,iteration,time.strftime('%Y-%m-%d %X',time.localtime(time.time())))
            cmd1 = "./waf --run 'dce-python-hello --langChoice=" + str(langChoice) +" --loopCount=" + str(loopCount) + " --iteration=" + str(iteration) + "'"
            print cmd1
            p1 = subprocess.Popen(shlex.split(cmd1),stdout=out4)
            while True:
                try:
                    pid_pox = subprocess.check_output(["pgrep", "-f", "/dce-python-hello"])
