...
std::cout << langChoice << "\t" << loopCount << "\t" << numBytes << "\t" << numNodes << "\t";
  ...
  oss.str(""); connInterfaces.at(1).GetAddress(0).Print(oss);
  ...
  ApplicationContainer serverClients;
  for (int i = 1; i < numNodes; ++i)
  {
    switch (langChoice)
    {
      case GCC:
        dce.SetBinary ("server_client_gcc");
        dce.ResetArguments ();
        dce.ResetEnvironment ();
        break;
      case CLANG:
        dce.SetBinary ("server_client_clang");
        dce.ResetArguments ();
        dce.ResetEnvironment ();
        break;
      case PYTHON:
        ...
        dce.AddEnvironment ("PYTHONPATH", "/:/python2.7:/pox:/ryu");
        dce.AddArgument ("-S");
        dce.AddArgument ("client_server.py");
        break;
      case JAVA:
        dce.SetBinary ("java");
        dce.ResetArguments ();
        ...
        dce.AddArgument ("-cp");
        dce.AddArgument ("/");
        dce.AddArgument ("TCPClientServer");
        break;
      default:
        std::cout << "Invalid language/compiler choice" << std::endl;
        return 1;
      }
    oss.str(""); connInterfaces.at((i+1)%numNodes).GetAddress(0).Print(oss);
    dce.AddArgument (oss.str());

    oss.str(""); oss << portStart++;
    dce.AddArgument (oss.str());

    oss.str(""); oss << loopCount*numBytes;
    dce.AddArgument (oss.str());

    ApplicationContainer serverApp = dce.Install (connNodes.Get (i));
    serverApp.Start (Seconds (1.0));
    serverClients.Add(serverApp);
  }

  ApplicationContainer sinkApps;
  PacketSinkHelper sink ("ns3::TcpSocketFactory",
                         InetSocketAddress (Ipv4Address::GetAny (), portStart));
  sink.SetAttribute ("MaxBytes", UintegerValue(numBytes*loopCount));
  ...
  std::cout << Simulator::Now().GetSeconds() << " " << d1 << " " << totalRx << " " << ReportMemoryUsageMB ()<< " " << getpid () << std::endl;
  Simulator::Destroy ();
}
